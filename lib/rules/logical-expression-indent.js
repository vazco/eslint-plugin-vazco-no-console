...
module.exports = {
    ...
    create(context) {
        ...
        function expectedExpressionIndent(node, parentIndent) {
            ...
            const parentExpression = expressionIndentStore.find(({start, end}) =>
                start <= node.start && node.end <= end
            )

            if (parentExpression) {
                return parentExpression.indent;
            }
            ...
            expressionIndentStore.push({start: node.start, end: node.end, indent: expressionIndent})
        ...
        return {
            IfStatement(node) {
                // This will be handled in definition for LogicalExpression, check below
                if (node.test.type === 'LogicalExpression') {
                    return;
                }

                // Skip inline conditionals
                if (isSingleLineNode(node)) {
                    return;
                }

                checkNodeIndent(node.test, getNodeIndent(node).goodChar + indentSize);
            },

            LogicalExpression(node) {
                // This should be handled by other rule(?)
                if (node.parent.type === 'ExpressionStatement') {
                    return;
                }
                // This should be handled by other rule(?)
                if (isSingleLineNode(node.parent)) {
                    return;
                }

                const expressionIndent = expectedExpressionIndent(node);

                checkNodeIndent(node.left, expressionIndent);
                checkNodeIndent(node.right, expressionIndent);
            }
        };
    }
};
